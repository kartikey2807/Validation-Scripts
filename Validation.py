...
class Runner(object):
    ...
    def runner(file_object):
        ...
        for t in kpi.keys():
            ...
            if len(i[0]) == 4:
                for j in i:
                    Lx_result_array.append([t, j[0], str(j[1]), str(j[2]), str(j[3])]) ## 4
            else:
                for j in i:
                    Lx_result_array.append([t, j[0], str(j[1]), str(j[2]), ""]) ## 3 params

        Lx_result = pd.DataFrame(numpy.asarray(Lx_result_array), index = None , columns = [
            'KPI', 'Plans', 'L5_Numbers', 'L3_Numbers', 'L2_Numbers'])
        ...
        resx = requests.get(url = f"http://{host}:{port}/collection/credential", params = {
            "datastore_name": "invcr_prod", "user_id": "dap_user"})
        ...
        s3_bucket.put_object(Body = csv_buffer.getvalue(), Key = output_file + file_upload)
